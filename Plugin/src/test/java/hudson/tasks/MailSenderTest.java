...
public class MailSenderTest {
    ...
    public void testIncludeUpstreamCulprits() throws Exception {
        ...
        MailSender sender = new MailSender("", false, false, "", false, false, false, false, "UTF-8", upstreamProjects);
    ...
    @Test public void forbiddenMail() throws Exception {
        ...
        assertEquals("authorized@mycorp", new MailSender("", false, true, "", false, false, false, false).getUserEmailList(listener, build));
        ...
        try {
            ...
            assertEquals("authorized@mycorp,unauthorized@mycorp", new MailSender("", false, true, "", false, false, false, false).getUserEmailList(listener, build));
            listener.getLogger().flush();
            ...
            try {
                sw = new StringWriter();
                listener = new StreamTaskListener(sw);
                assertEquals("authorized@mycorp,unauthorized@mycorp,someone@nowhere.net", new MailSender("", false, true, "", false, false, false, false).getUserEmailList(listener, build));
                listener.getLogger().flush();
                assertThat(sw.toString(), containsString(Messages.MailSender_warning_user_without_read("unauthorized@mycorp", "prj #1")));
